...
if (!jwtSecret) {
    console.error('❌ JWT_SECRET is not set. Refusing to start without a secure signing key.');
    process.exit(1);
}
...
const authenticateToken = (req, res, next) => {
    ...
    const cookieToken = token || getTokenFromCookie(req);

    if (cookieToken == null) {
        return res.status(401).json({ message: 'Authentication required: Missing token.' });
    }

    jwt.verify(cookieToken, jwtSecret, (err, user) => {
        if (err) {
